...
pub async fn get_tracks(spotify_ids: Vec<String>, session: &Session) -> Result<Vec<Track>> {
    ...
    for id in spotify_ids {
        ...
        let id: SpotifyId = parse_uri_or_url(&id).ok_or(anyhow::anyhow!("Invalid track"))?;
        println!("id: {:?}", id.item_type);
...
impl Track {
    ...
    pub async fn metadata(&self, session: &Session) -> Result<TrackMetadata> {
        ...
        for artist in &metadata.artists.0 {
            artists.push(
    ...
    pub async fn info(&self, session: &Session) -> Result<librespot::metadata::Track> {
        let info = librespot::metadata::Track::get(session, &self.id)
            .await
            .map_err(|_| anyhow::anyhow!("Failed to get metadata"))?;
...
impl TrackCollection for Album {
    async fn get_tracks(&self, session: &Session) -> Vec<Track> {
        let album: librespot::metadata::Album = librespot::metadata::Album::get(session, &self.id)
            .await
...
impl TrackCollection for Playlist {
    async fn get_tracks(&self, session: &Session) -> Vec<Track> {
        ...
        playlist.tracks().map(|track| Track::from_id(*track)).collect()
    }
}
